 ...
 */
const publicKeyBytesToArray = (publicKey) => ({
 ...
 */
const publicKeyArrayToBytes = (publicKey) => ({
 ...
 */
const transformKeyBytesToArray = (transformKey) => ({
 ...
 */
const transformKeyArrayToBytes = (transformKey) => ({
...
export class Api256 {
     ...
     */
    transformBlocksBytesToArray(transformBlocks) {
     ...
     * to the caller.
     */
    transformBlocksArrayToBytes(transformBlocks) {
        return transformBlocks.map((block) => ({
     ...
     */
    encryptedValueBytesToArray(encryptedValue) {
        return {
     ...
     */
    encryptedValueArrayToBytes(encryptedValue) {
        return {
     ...
     */
    ed25519Sign(privateSigningKey, message) {
    ...
    transform(encryptedValue, transformKey, privateSigningKey) {
        const transformedEncryptedValue = this.api.transform(this.encryptedValueBytesToArray(encryptedValue), transformKeyBytesToArray(transformKey), privateSigningKey);
    ...
    schnorrVerify(publicKey, augmentedPrivateKey, message, signature) {
        return this.api.schnorrVerify(publicKeyBytesToArray(publicKey), augmentedPrivateKey, message, signature);
    }
    /**
 ...
 */
export const transformKeyToBytes256 = (transformKey) => Recrypt.transformKeyToBytes256(transformKeyBytesToArray(transformKey));
/**
 * Augment the provided transform key object with the provided private key. Returns a new, augmented TransformKey
 */
export const augmentTransformKey256 = (transformKey, privateKey) => transformKeyArrayToBytes(Recrypt.augmentTransformKey256(transformKeyBytesToArray(transformKey), privateKey));
/**
 * Augment the provided public key object with another public key object. Returns the augmented PublicKey.
 */
export const augmentPublicKey256 = (currentPublicKey, otherPublicKey) => {
 ...
 */
export const addPrivateKeys = (privateKeyA, privateKeyB) => Recrypt.addPrivateKeys(privateKeyA, privateKeyB);
/**
 * Create a new private key by subtracting the provided private keys.
 */
export const subtractPrivateKeys = (privateKeyA, privateKeyB) => Recrypt.subtractPrivateKeys(privateKeyA, privateKeyB);
/**
...
//# sourceMappingURL=Api256Shim.js.map